...
export default function Init_I_drivingModeBg(scene, camera) {
  ...
  loader.load(streeModel, (gltf) => {
    ...
    gltf.scene.scale.set(sceneParams.size, sceneParams.size, sceneParams.size);
    ...
    pointMaterial = new THREE.ShaderMaterial({
      ...
      void main() {
        ...
        pos += randomDir * t * 20.0; // 控制扰动范围（可调）

        float r = length(pos.xz); // 与中心的距离
        float waveFront = waveTime * 800.0; // 控制扩散速度
        float waveWidth = 800.0; // 波的宽度范围
        vR = r;
        ...
        if (r < waveFront && r > waveFront - waveWidth) {
          pos.y += lift * vR * smoothstep(5500.0,3500.,vR);
        } 
          
        float offset = 1000.*sin(waveTime + dot(pos.xyz, randomDir));
