...
onMounted(() => {
  ...
  curvesInstence.setMaterial(
    curvesInstence.allLinesMesh.length - 5,
    redMaterial,
    () => {
      curvesInstence.otherMaterial.uniforms.vTime.value =
        curvesInstence.clock.getElapsedTime();
    }
  );
...
const handleClick = (mode) => {
  ...
  } else if (currentMode == "" && mode === "I_driving") {
    currentMode = mode;
    su7world.changeI_drivingMode();
  } else if (currentMode == "I_driving" && mode === "I_driving") {
    currentMode = "";
    su7world.changeI_drivingModeToNomarl();
  }
};
</script>
...
<template>
  <div id="su7World">
    <div class="windBtn">
      ...
      <button class="btn" @click="handleClick('I_driving')">智驾</button>
    </div>
  </div>
</template>
